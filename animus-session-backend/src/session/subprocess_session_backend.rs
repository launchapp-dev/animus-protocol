//! Generic subprocess-spawning backend used as a fallback for unknown tools
//! and as the launch primitive every per-CLI backend builds on top of.
...
/// registered for the requested tool.
...
impl SubprocessSessionBackend {
    /// Construct a new subprocess backend.
    pub fn new() -> Self {
        Self
    }
}
