//! Runtime launch parsing and argv normalization shared by every backend.
...
/// A fully-resolved CLI invocation: command, args, env overlay, and whether to
/// pipe the prompt over stdin.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LaunchInvocation {
    /// Command to spawn (typically a binary name resolved against PATH).
    pub command: String,
    /// Argument vector passed to the command.
    pub args: Vec<String>,
    /// Additional environment variables to inject (merged on top of inherited env).
    pub env: BTreeMap<String, String>,
    /// If true, the prompt text is written to the child's stdin instead of an argv slot.
    pub prompt_via_stdin: bool,
}

/// Look up a binary in PATH and return its full path if found.
///
/// Uses `which` on Unix and `where` on Windows. Returns `None` on other platforms.
...
/// Parse a tool string (e.g. `"claude"`, `"open-code"`) into a [`CliType`].
...
/// True if the given tool name is recognized as one of the supported CLIs.
...
/// Ensure machine-parseable output flags are present for the recognized CLIs.
...
/// Parse a runtime contract JSON envelope into a [`LaunchInvocation`].
///
/// Returns `Ok(None)` if no contract is supplied or it doesn't contain a launch
/// block. Returns `Err` if the contract is malformed.
...
mod tests {
    ...
    fn parse_launch_preserves_environment_variables() {
        ...
        let launch = parse_launch_from_runtime_contract(Some(&contract))
            ...
            .expect("launch should be present");
        assert_eq!(
            launch.env.get("SKILL_MODE").map(String::as_str),
            Some("review")
        );
