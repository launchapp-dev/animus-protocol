...
//!
//! ```text
//! cargo run -p animus-transport-protocol --bin animus-transport-protocol-export-schema -- [--out <dir>]
//! ```
//!
//! Mirrors the binary in `animus-plugin-protocol`. The default output
//! directory is `schemas/animus-transport-protocol/` resolved relative to
...
//! - `TransportBackend` — a Rust trait, not a serializable wire type.
...
use animus_transport_protocol::{TransportConfig, TransportInfo, TransportSchema};
...
fn default_out_dir() -> PathBuf {
    ...
    base.join("schemas").join("animus-transport-protocol")
}
...
pub fn all_schemas() -> Vec<(&'static str, Schema)> {
    vec![
        ("TransportConfig", schema_for!(TransportConfig)),
        ("TransportInfo", schema_for!(TransportInfo)),
        ("TransportSchema", schema_for!(TransportSchema)),
    ]
}
...
pub fn export_to(out_dir: &Path) -> std::io::Result<usize> {
    ...
    let bundle = serde_json::json!({
        ...
        "title": "animus-transport-protocol",
        "$defs": defs,
    });
...
mod tests {
    ...
    #[test]
    fn transport_config_emits_object_type() {
        let schema = schema_for!(TransportConfig);
        ...
        assert!(
            type_field == Value::String("object".to_string())
                || type_field
                    ...
                    .unwrap_or(false),
            "TransportConfig schema should report object type, got {type_field}"
        );
    }
}
