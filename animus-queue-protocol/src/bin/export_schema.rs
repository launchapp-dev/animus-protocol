...
//!
//! ```text
//! cargo run -p animus-queue-protocol --bin animus-queue-protocol-export-schema -- [--out <dir>]
//! ```
//!
//! Mirrors the binary in `animus-plugin-protocol`. The default output
//! directory is `schemas/animus-queue-protocol/` resolved relative to the
//! workspace root. One file is written per type plus an `_all.json` bundle
//! for tooling that wants a single artifact.
//!
//! Rust-only constructs (the `status` / `completion_status` /
//! `error_codes` const modules) are intentionally omitted: they are not
//! wire message types. The wire-level error shape is
...
fn default_out_dir() -> PathBuf {
    ...
    base.join("schemas").join("animus-queue-protocol")
}
...
pub fn all_schemas() -> Vec<(&'static str, Schema)> {
    vec![
        ("QueueEnqueueRequest", schema_for!(QueueEnqueueRequest)),
        ("QueueEnqueueResponse", schema_for!(QueueEnqueueResponse)),
        ("QueueListRequest", schema_for!(QueueListRequest)),
        ("QueueListResponse", schema_for!(QueueListResponse)),
        ("QueueEntry", schema_for!(QueueEntry)),
        ...
        ("QueueDropRequest", schema_for!(QueueDropRequest)),
        ("QueueReorderRequest", schema_for!(QueueReorderRequest)),
        ("QueueReorderResponse", schema_for!(QueueReorderResponse)),
        (
            "QueueMarkAssignedRequest",
            schema_for!(QueueMarkAssignedRequest),
        ),
        (
            "QueueReleasePendingParams",
            schema_for!(QueueReleasePendingParams),
        ),
        (
        ...
        (
            ...
            schema_for!(QueueCompletionRequest),
        ),
        ("QueueCapabilities", schema_for!(QueueCapabilities)),
    ]
}
...
mod tests {
    ...
    #[test]
    fn queue_entry_emits_object_type() {
        let schema = schema_for!(QueueEntry);
        ...
        assert!(
            type_field == Value::String("object".to_string())
                || type_field
                    ...
                    .unwrap_or(false),
            "QueueEntry schema should report object type, got {type_field}"
        );
    }
}
