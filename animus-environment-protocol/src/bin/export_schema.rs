...
//!
//! ```text
//! cargo run -p animus-environment-protocol --bin animus-environment-protocol-export-schema -- [--out <dir>]
//! ```
//!
//! Mirrors the binary in `animus-plugin-protocol`. The default output directory
//! is `schemas/animus-environment-protocol/` resolved relative to the workspace
//! root. One file is written per type plus an `_all.json` bundle for tooling
//! that wants a single artifact.
//!
//! The wire-level error shape is `animus_plugin_protocol::RpcError`, which is
//! exported by the sibling `animus-plugin-protocol` schema binary and so is not
//! re-emitted here.
...
fn default_out_dir() -> PathBuf {
    ...
    base.join("schemas").join("animus-environment-protocol")
}
...
pub fn all_schemas() -> Vec<(&'static str, Schema)> {
    vec![
        ("RepoRef", schema_for!(RepoRef)),
        ("EnvironmentSpec", schema_for!(EnvironmentSpec)),
        ("EnvironmentHandle", schema_for!(EnvironmentHandle)),
        ("PrepareRequest", schema_for!(PrepareRequest)),
        ("PrepareResponse", schema_for!(PrepareResponse)),
...
pub fn export_to(out_dir: &Path) -> std::io::Result<usize> {
    ...
    let bundle = serde_json::json!({
        ...
        "title": "animus-environment-protocol",
        "$defs": defs,
    });
...
mod tests {
    ...
    #[test]
    fn environment_spec_emits_object_type() {
        let schema = schema_for!(EnvironmentSpec);
        ...
        assert!(
            type_field == Value::String("object".to_string())
                || type_field
                    ...
                    .unwrap_or(false),
            "EnvironmentSpec schema should report object type, got {type_field}"
        );
    }
}
