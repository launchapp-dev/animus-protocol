...
//!
//! ```text
//! cargo run -p animus-memory-store-protocol --bin animus-memory-store-protocol-export-schema -- [--out <dir>]
//! ```
//!
//! Mirrors the binary in `animus-plugin-protocol`. The default output
//! directory is `schemas/animus-memory-store-protocol/` resolved relative
//! to the workspace root. One file is written per type plus an `_all.json`
//! bundle for tooling that wants a single artifact.
//!
//! The `error_codes` const module is intentionally omitted: it is not a
//! wire message type. The wire-level error shape is
...
use animus_memory_store_protocol::{
    DeleteScopeRequest, DeleteScopeResponse, GetMemoryRequest, GetMemoryResponse,
    ListScopesRequest, ListScopesResponse, MemoryQueryResult, MemoryScope, MemoryStoreCapabilities,
    PutMemoryRequest, PutMemoryResponse, QueryMemoryRequest, QueryMemoryResponse,
};
...
fn default_out_dir() -> PathBuf {
    ...
    base.join("schemas").join("animus-memory-store-protocol")
}
...
pub fn all_schemas() -> Vec<(&'static str, Schema)> {
    vec![
        ("MemoryScope", schema_for!(MemoryScope)),
        ("PutMemoryRequest", schema_for!(PutMemoryRequest)),
        ("PutMemoryResponse", schema_for!(PutMemoryResponse)),
        ("GetMemoryRequest", schema_for!(GetMemoryRequest)),
        ("GetMemoryResponse", schema_for!(GetMemoryResponse)),
        ("QueryMemoryRequest", schema_for!(QueryMemoryRequest)),
        ("QueryMemoryResponse", schema_for!(QueryMemoryResponse)),
        ("MemoryQueryResult", schema_for!(MemoryQueryResult)),
        ("ListScopesRequest", schema_for!(ListScopesRequest)),
        ("ListScopesResponse", schema_for!(ListScopesResponse)),
        ("DeleteScopeRequest", schema_for!(DeleteScopeRequest)),
        ("DeleteScopeResponse", schema_for!(DeleteScopeResponse)),
        (
            "MemoryStoreCapabilities",
            schema_for!(MemoryStoreCapabilities),
        ),
    ]
}
...
mod tests {
    ...
    #[test]
    fn put_request_emits_object_type() {
        let schema = schema_for!(PutMemoryRequest);
        ...
        assert!(
            type_field == Value::String("object".to_string())
                || type_field
                    ...
                    .unwrap_or(false),
            "PutMemoryRequest schema should report object type, got {type_field}"
        );
    }
}
