...
//!
//! ```text
//! cargo run -p animus-provider-protocol --bin animus-provider-protocol-export-schema -- [--out <dir>]
//! ```
//!
//! Mirrors the binary in `animus-plugin-protocol`. The default output
//! directory is `schemas/animus-provider-protocol/` resolved relative to
//! the workspace root. One file is written per type plus an `_all.json`
//! bundle for tooling that wants a single artifact.
//!
//! Omitted intentionally:
//! - `AgentResumeRequest` — a type alias of `AgentRunRequest`; the wire
//!   shape is identical and is already emitted under `AgentRunRequest`.
//! - `BackendError` — carries an opaque `anyhow::Error` and is a Rust-side
//!   error type, not a wire message. The wire-level error shape is
//!   `animus_plugin_protocol::RpcError`, exported by the sibling
//!   `animus-plugin-protocol` schema binary.
//! - `NotificationSink` / `ProviderBackend` — a Rust callback wrapper and a
//!   Rust trait, neither of which is a serializable wire type.
...
use animus_provider_protocol::{
    AgentCancelRequest, AgentNotification, AgentRunRequest, AgentRunResponse, ProviderCapabilities,
...
fn default_out_dir() -> PathBuf {
    ...
    base.join("schemas").join("animus-provider-protocol")
}
...
pub fn all_schemas() -> Vec<(&'static str, Schema)> {
    vec![
        ("ProviderManifest", schema_for!(ProviderManifest)),
        ("ProviderCapabilities", schema_for!(ProviderCapabilities)),
        ("AgentRunRequest", schema_for!(AgentRunRequest)),
        ("AgentCancelRequest", schema_for!(AgentCancelRequest)),
        ("AgentRunResponse", schema_for!(AgentRunResponse)),
        ("TokenUsage", schema_for!(TokenUsage)),
        ("AgentNotification", schema_for!(AgentNotification)),
    ]
}
...
pub fn export_to(out_dir: &Path) -> std::io::Result<usize> {
    ...
    let bundle = serde_json::json!({
        ...
        "title": "animus-provider-protocol",
        "$defs": defs,
    });
...
mod tests {
    ...
    #[test]
    fn agent_run_request_emits_object_type() {
        let schema = schema_for!(AgentRunRequest);
        ...
        assert!(
            type_field == Value::String("object".to_string())
                || type_field
                    ...
                    .unwrap_or(false),
            "AgentRunRequest schema should report object type, got {type_field}"
        );
    }
}
