...
//!
//! ```text
//! cargo run -p animus-notifier-protocol --bin animus-notifier-protocol-export-schema -- [--out <dir>]
//! ```
//!
//! Mirrors the binary in `animus-plugin-protocol`. The default output
//! directory is `schemas/animus-notifier-protocol/` resolved relative to
//! the workspace root. One file is written per type plus an `_all.json`
//! bundle for tooling that wants a single artifact.
//!
//! `NotifierBackendError` is intentionally omitted: it carries an opaque
//! `anyhow::Error` payload and is a Rust-side error type rather than a
//! wire message. The wire-level error shape is
...
use animus_notifier_protocol::{
    DaemonEventRecord, NotifierFlushParams, NotifierFlushResult, NotifierLifecycleEvent,
    NotifierNotifyParams, NotifierNotifyResult, NotifierSchema,
};
...
fn default_out_dir() -> PathBuf {
    ...
    base.join("schemas").join("animus-notifier-protocol")
}
...
pub fn all_schemas() -> Vec<(&'static str, Schema)> {
    vec![
        ("DaemonEventRecord", schema_for!(DaemonEventRecord)),
        ("NotifierNotifyParams", schema_for!(NotifierNotifyParams)),
        ("NotifierNotifyResult", schema_for!(NotifierNotifyResult)),
        (
            "NotifierLifecycleEvent",
            schema_for!(NotifierLifecycleEvent),
        ),
        ("NotifierFlushParams", schema_for!(NotifierFlushParams)),
        ("NotifierFlushResult", schema_for!(NotifierFlushResult)),
        ("NotifierSchema", schema_for!(NotifierSchema)),
    ]
}
...
mod tests {
    ...
    #[test]
    fn notify_params_emits_object_type() {
        let schema = schema_for!(NotifierNotifyParams);
        ...
        assert!(
            type_field == Value::String("object".to_string())
                || type_field
                    ...
                    .unwrap_or(false),
            "NotifierNotifyParams schema should report object type, got {type_field}"
        );
    }
}
