...
//!
//! ```text
//! cargo run -p animus-trigger-protocol --bin animus-trigger-protocol-export-schema -- [--out <dir>]
//! ```
//!
//! Mirrors the binary in `animus-plugin-protocol`. The default output
//! directory is `schemas/animus-trigger-protocol/` resolved relative to
...
//! - `TriggerStream` / `TriggerBackend` — a Rust stream alias and a Rust
//!   trait, neither serializable.
//! - The `trigger/watch` and `trigger/ack` param shapes are owned by
//!   `animus-plugin-protocol` (`TriggerWatchParams` / `TriggerAckParams`)
//!   and exported by its schema binary.
...
fn default_out_dir() -> PathBuf {
    ...
    base.join("schemas").join("animus-trigger-protocol")
}
...
pub fn all_schemas() -> Vec<(&'static str, Schema)> {
    vec![
        ...
        ("TriggerSchema", schema_for!(TriggerSchema)),
    ]
}
...
mod tests {
    ...
    #[test]
    fn trigger_event_emits_object_type() {
        let schema = schema_for!(TriggerEvent);
        ...
        assert!(
            type_field == Value::String("object".to_string())
                || type_field
                    ...
                    .unwrap_or(false),
            "TriggerEvent schema should report object type, got {type_field}"
        );
    }
}
