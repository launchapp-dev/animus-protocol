...
//!
//! ```text
//! cargo run -p animus-log-storage-protocol --bin animus-log-storage-protocol-export-schema -- [--out <dir>]
//! ```
//!
//! Mirrors the binary in `animus-plugin-protocol`. The default output
//! directory is `schemas/animus-log-storage-protocol/` resolved relative to
//! the workspace root. One file is written per type plus an `_all.json`
//! bundle for tooling that wants a single artifact.
//!
//! Note: `LogStorageSchema::max_query_window` / `retention_hint` are
//! `chrono::Duration` on the Rust side but serialize as a signed
//! millisecond integer on the wire (see the `duration_ms_opt` serde
//! helper). The schema reflects that wire shape via
...
//! - `LogStream` / `LogStorageBackend` — a Rust stream alias and a Rust
//!   trait, neither serializable.
...
fn default_out_dir() -> PathBuf {
    ...
    base.join("schemas").join("animus-log-storage-protocol")
}
...
pub fn all_schemas() -> Vec<(&'static str, Schema)> {
    vec![
        ...
        ("LogLevel", schema_for!(LogLevel)),
        ("LogSource", schema_for!(LogSource)),
        ("LogQuery", schema_for!(LogQuery)),
        ("LogQueryResult", schema_for!(LogQueryResult)),
        ("LogStorageSchema", schema_for!(LogStorageSchema)),
        ("SupportsFiltering", schema_for!(SupportsFiltering)),
    ]
}
...
pub fn export_to(out_dir: &Path) -> std::io::Result<usize> {
    ...
    let bundle = serde_json::json!({
        ...
        "title": "animus-log-storage-protocol",
        "$defs": defs,
    });
...
mod tests {
    ...
    #[test]
    fn log_entry_emits_object_type() {
        let schema = schema_for!(LogEntry);
        ...
        assert!(
            type_field == Value::String("object".to_string())
                || type_field
                    ...
                    .unwrap_or(false),
            "LogEntry schema should report object type, got {type_field}"
        );
    }
}
