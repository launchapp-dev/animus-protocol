...
//!
//! ```text
//! cargo run -p animus-workflow-runner-protocol --bin animus-workflow-runner-protocol-export-schema -- [--out <dir>]
//! ```
//!
//! Mirrors the binary in `animus-plugin-protocol`. The default output
//! directory is `schemas/animus-workflow-runner-protocol/` resolved
//! relative to the workspace root. One file is written per type plus an
//! `_all.json` bundle for tooling that wants a single artifact.
//!
//! The `workflow_status` / `phase_status` / `error_codes` const modules
//! (and `workflow_status::Parsed`, a Rust-only parse helper) are
//! intentionally omitted: they are not wire message types. The wire-level
//! error shape is `animus_plugin_protocol::RpcError`, exported by the
//! sibling `animus-plugin-protocol` schema binary.
...
fn default_out_dir() -> PathBuf {
    ...
    base.join("schemas").join("animus-workflow-runner-protocol")
}
...
pub fn all_schemas() -> Vec<(&'static str, Schema)> {
    vec![
        (
            ...
            schema_for!(WorkflowExecuteRequest),
        ),
        ...
        (
            ...
            schema_for!(WorkflowPhaseRunResult),
        ),
        (
            "WorkflowRunnerManifest",
            schema_for!(WorkflowRunnerManifest),
        ),
        (
            "WorkflowRunnerCapabilities",
            schema_for!(WorkflowRunnerCapabilities),
        ),
    ]
}
...
pub fn export_to(out_dir: &Path) -> std::io::Result<usize> {
    ...
    let bundle = serde_json::json!({
        ...
        "title": "animus-workflow-runner-protocol",
        "$defs": defs,
    });
...
mod tests {
    ...
    #[test]
    fn execute_request_emits_object_type() {
        let schema = schema_for!(WorkflowExecuteRequest);
